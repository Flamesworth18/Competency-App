...
@Component({
  selector: 'app-extension-five-a',
  templateUrl: './extension-five-a.component.html',
  styleUrls: ['./extension-five-a.component.scss']
})
export class ExtensionFiveAComponent implements OnInit {
  ...
  navigateToPagination(route: string){
    ...
    var basicOptions = this.userExtensionCompetency.fiveBasicOptions;
    var intermediateOptions = this.userExtensionCompetency.fiveIntermediateOptions;
    var advancedOptions = this.userExtensionCompetency.fiveAdvancedOptions;
    var superiorOptions = this.userExtensionCompetency.fiveSuperiorOptions;
    ...
    if(basicScore.length >= 6){
      this.userExtensionCompetency.fiveBasicResult.competent = true;
    }
    else if(basicScore.length >= 3 && basicScore.length <= 5){
      this.userExtensionCompetency.fiveBasicResult.partially = true;
    }
    else if(basicScore.length <= 2){
      this.userExtensionCompetency.fiveBasicResult.notYet = true;
    }
    //#endregion

    //#region INTERMEDIATE
    if(intermediateScore.length >= 6){
      this.userExtensionCompetency.fiveIntermediateResult.competent = true;
    }
    else if(intermediateScore.length >= 3 && intermediateScore.length <= 5){
      this.userExtensionCompetency.fiveIntermediateResult.partially = true;
    }
    else if(intermediateScore.length <= 2){
      this.userExtensionCompetency.fiveIntermediateResult.notYet = true;
    }
    //#endregion

    //#region ADVANCED
    if(advancedScore.length >= 6){
      this.userExtensionCompetency.fiveAdvancedResult.competent = true;
    }
    else if(advancedScore.length >= 3 && advancedScore.length <= 5){
      this.userExtensionCompetency.fiveAdvancedResult.partially = true;
    }
    else if(advancedScore.length <= 2){
      this.userExtensionCompetency.fiveAdvancedResult.notYet = true;
    }
    //#endregion

    //#region SUPERIOR
    if(superiorScore.length >= 6){
      this.userExtensionCompetency.fiveSuperiorResult.competent = true;
    }
    else if(superiorScore.length >= 3 && superiorScore.length <= 5){
      this.userExtensionCompetency.fiveSuperiorResult.partially = true;
    }
    else if(superiorScore.length <= 2){
      this.userExtensionCompetency.fiveSuperiorResult.notYet = true;
    }
    //#endregion
  ...
  addToBasicOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userExtensionCompetency.fiveBasicOptions.findIndex(o => o.id === option.id);
      this.userExtensionCompetency.fiveBasicOptions[optionIndex] = option;
    }
  }
  ...
  addToIntermediateOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userExtensionCompetency.fiveIntermediateOptions.findIndex(o => o.id === option.id);
      this.userExtensionCompetency.fiveIntermediateOptions[optionIndex] = option;
    }
  }
  ...
  addToAdvancedOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userExtensionCompetency.fiveAdvancedOptions.findIndex(o => o.id === option.id);
      this.userExtensionCompetency.fiveAdvancedOptions[optionIndex] = option;
    }
  }
  ...
  addToSuperiorOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userExtensionCompetency.fiveSuperiorOptions.findIndex(o => o.id === option.id);
      this.userExtensionCompetency.fiveSuperiorOptions[optionIndex] = option;
    }
  }
