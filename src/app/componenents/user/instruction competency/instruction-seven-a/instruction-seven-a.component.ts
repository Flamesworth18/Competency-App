...
@Component({
  selector: 'app-instruction-seven-a',
  templateUrl: './instruction-seven-a.component.html',
  styleUrls: ['./instruction-seven-a.component.scss']
})
export class InstructionSevenAComponent implements OnInit {
  ...
  navigateToPagination(route: string){
    ...
    var basicOptions = this.userInstructionCompetency.sevenBasicOptions;
    var intermediateOptions = this.userInstructionCompetency.sevenIntermediateOptions;
    var advancedOptions = this.userInstructionCompetency.sevenAdvancedOptions;
    var superiorOptions = this.userInstructionCompetency.sevenSuperiorOptions;
    ...
    if(basicScore.length >= 6){
      this.userInstructionCompetency.sevenBasicResult.competent = true;
    }
    else if(basicScore.length >= 3 && basicScore.length <= 5){
      this.userInstructionCompetency.sevenBasicResult.partially = true;
    }
    else if(basicScore.length <= 2){
      this.userInstructionCompetency.sevenBasicResult.notYet = true;
    }
    //#endregion

    //#region INTERMEDIATE
    if(intermediateScore.length >= 6){
      this.userInstructionCompetency.sevenIntermediateResult.competent = true;
    }
    else if(intermediateScore.length >= 3 && intermediateScore.length <= 5){
      this.userInstructionCompetency.sevenIntermediateResult.partially = true;
    }
    else if(intermediateScore.length <= 2){
      this.userInstructionCompetency.sevenIntermediateResult.notYet = true;
    }
    //#endregion

    //#region ADVANCED
    if(advancedScore.length >= 6){
      this.userInstructionCompetency.sevenAdvancedResult.competent = true;
    }
    else if(advancedScore.length >= 3 && advancedScore.length <= 5){
      this.userInstructionCompetency.sevenAdvancedResult.partially = true;
    }
    else if(advancedScore.length <= 2){
      this.userInstructionCompetency.sevenAdvancedResult.notYet = true;
    }
    //#endregion

    //#region SUPERIOR
    if(superiorScore.length >= 6){
      this.userInstructionCompetency.sevenSuperiorResult.competent = true;
    }
    else if(superiorScore.length >= 3 && superiorScore.length <= 5){
      this.userInstructionCompetency.sevenSuperiorResult.partially = true;
    }
    else if(superiorScore.length <= 2){
      this.userInstructionCompetency.sevenSuperiorResult.notYet = true;
    }
    //#endregion
  ...
  addToBasicOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userInstructionCompetency.sevenBasicOptions.findIndex(o => o.id === option.id);
      this.userInstructionCompetency.sevenBasicOptions[optionIndex] = option;
    }
  }
  ...
  addToIntermediateOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userInstructionCompetency.sevenIntermediateOptions.findIndex(o => o.id === option.id);
      this.userInstructionCompetency.sevenIntermediateOptions[optionIndex] = option;
    }
  }
  ...
  addToAdvancedOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userInstructionCompetency.sevenAdvancedOptions.findIndex(o => o.id === option.id);
      this.userInstructionCompetency.sevenAdvancedOptions[optionIndex] = option;
    }
  }
  ...
  addToSuperiorOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userInstructionCompetency.sevenSuperiorOptions.findIndex(o => o.id === option.id);
      this.userInstructionCompetency.sevenSuperiorOptions[optionIndex] = option;
    }
  }
