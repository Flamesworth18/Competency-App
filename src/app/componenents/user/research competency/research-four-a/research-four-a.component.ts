...
@Component({
  selector: 'app-research-four-a',
  templateUrl: './research-four-a.component.html',
  styleUrls: ['./research-four-a.component.scss']
})
export class ResearchFourAComponent implements OnInit {
  ...
  navigateToPagination(route: string){
    ...
    var basicOptions = this.userResearchCompetency.fourBasicOptions;
    var intermediateOptions = this.userResearchCompetency.fourIntermediateOptions;
    var advancedOptions = this.userResearchCompetency.fourAdvancedOptions;
    var superiorOptions = this.userResearchCompetency.fourSuperiorOptions;
    ...
    if(basicScore.length >= 6){
      this.userResearchCompetency.fourBasicResult.competent = true;
    }
    else if(basicScore.length >= 3 && basicScore.length <= 5){
      this.userResearchCompetency.fourBasicResult.partially = true;
    }
    else if(basicScore.length <= 2){
      this.userResearchCompetency.fourBasicResult.notYet = true;
    }
    //#endregion

    //#region INTERMEDIATE
    if(intermediateScore.length >= 6){
      this.userResearchCompetency.fourIntermediateResult.competent = true;
    }
    else if(intermediateScore.length >= 3 && intermediateScore.length <= 5){
      this.userResearchCompetency.fourIntermediateResult.partially = true;
    }
    else if(intermediateScore.length <= 2){
      this.userResearchCompetency.fourIntermediateResult.notYet = true;
    }
    //#endregion

    //#region ADVANCED
    if(advancedScore.length >= 6){
      this.userResearchCompetency.fourAdvancedResult.competent = true;
    }
    else if(advancedScore.length >= 3 && advancedScore.length <= 5){
      this.userResearchCompetency.fourAdvancedResult.partially = true;
    }
    else if(advancedScore.length <= 2){
      this.userResearchCompetency.fourAdvancedResult.notYet = true;
    }
    //#endregion

    //#region SUPERIOR
    if(superiorScore.length >= 6){
      this.userResearchCompetency.fourSuperiorResult.competent = true;
    }
    else if(superiorScore.length >= 3 && superiorScore.length <= 5){
      this.userResearchCompetency.fourSuperiorResult.partially = true;
    }
    else if(superiorScore.length <= 2){
      this.userResearchCompetency.fourSuperiorResult.notYet = true;
    }
    //#endregion
  ...
  addToBasicOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userResearchCompetency.fourBasicOptions.findIndex(o => o.id === option.id);
      this.userResearchCompetency.fourBasicOptions[optionIndex] = option;
    }
  }
  ...
  addToIntermediateOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userResearchCompetency.fourIntermediateOptions.findIndex(o => o.id === option.id);
      this.userResearchCompetency.fourIntermediateOptions[optionIndex] = option;
    }
  }
  ...
  addToAdvancedOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userResearchCompetency.fourAdvancedOptions.findIndex(o => o.id === option.id);
      this.userResearchCompetency.fourAdvancedOptions[optionIndex] = option;
    }
  }
  ...
  addToSuperiorOptionList(event: any, option: any) {
    ...
    if ( event.target.checked ) {
      var optionIndex = this.userResearchCompetency.fourSuperiorOptions.findIndex(o => o.id === option.id);
      this.userResearchCompetency.fourSuperiorOptions[optionIndex] = option;
    }
  }
